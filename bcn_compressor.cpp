...
std::vector<uint8_t> BCnCompressor::CompressRGBA(const uint8_t* rgbaData, uint32_t width, uint32_t height, uint8_t channels, BCFormat format, int numThreads, float quality, uint8_t alphaThreshold)
{
...
std::vector<uint8_t> BCnCompressor::CompressHDR(const float* rgbaData, uint32_t width, uint32_t height, BCFormat format, int numThreads, float quality)
{
    CMP_Texture srcTexture = {};
    srcTexture.dwSize = sizeof(CMP_Texture);
    srcTexture.dwWidth = width;
    ...
    destTexture.dwWidth = width;
    destTexture.dwHeight = height;
    destTexture.format = static_cast<CMP_FORMAT>(GetCMPFormat(format)); // Should be BC6H
...
std::vector<uint8_t> BCnCompressor::DecompressToRGBA(const uint8_t* bcData, uint32_t width, uint32_t height, BCFormat format)
{
    CMP_Texture srcTexture = {};
    srcTexture.dwSize = sizeof(CMP_Texture);
    srcTexture.dwWidth = width;
...
std::vector<float> BCnCompressor::DecompressToRGBAF(const uint8_t* bcData, uint32_t width, uint32_t height, BCFormat format)
{
    CMP_Texture srcTexture = {};
    srcTexture.dwSize = sizeof(CMP_Texture);
    srcTexture.dwWidth = width;
