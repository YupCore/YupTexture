...
float VQEncoder::OklabFloatBlockDistanceSq_SIMD(const OklabFloatBlock& labA, const OklabFloatBlock& labB) const
{
    ...
    const __m256 weight = _mm256_set_ps(1.0f, 1.0f, 2.0f, 1.0f, 1.0f, 1.0f, 2.0f, 1.0f);
...
void VQEncoder::RgbToCielab(const uint8_t* rgb, float* lab) const {
...
void VQEncoder::CielabToRgb(const float* lab, uint8_t* rgb) const {
...
std::vector<uint32_t> VQEncoder::QuantizeBlocks(const std::vector<std::vector<uint8_t>>& rgbaBlocks, const std::vector<std::vector<uint8_t>>& rgbaCentroids, const CompressionParams& params) {
...
std::vector<uint8_t> VQEncoder::CompressSingleBlockHDR(const std::vector<float>& rgbaBlock, int numThreads, float quality) {
...
#pragma omp parallel for num_threads(params.numThreads)
    for (int64_t i = 0; i < numBlocks; ++i) {
        // --- Enable ACES tonemapping for HDR->Oklab conversion ---
...
#pragma omp parallel for num_threads(params.numThreads)
    for (int64_t i = 0; i < config.codebookSize; ++i) {
        ...
        // --- Apply INVERSE ACES tonemapper to restore HDR values ---
...
#pragma omp parallel for num_threads(params.numThreads)
    for (int64_t i = 0; i < config.codebookSize; ++i) {
        // --- Compress codebook entries with max quality for best results ---
        finalCodebook.entries[i] = CompressSingleBlockHDR(outRgbaCentroids[i], params.numThreads, 1.0f);
    }
    return finalCodebook;
}

std::vector<uint32_t> VQEncoder::QuantizeBlocksHDR(const std::vector<std::vector<float>>& rgbaFloatBlocks, const std::vector<std::vector<float>>& rgbaCentroids, const CompressionParams& params) {
...
#pragma omp parallel for num_threads(params.numThreads)
    for (int64_t i = 0; i < codebookSize; ++i) {
        // --- Enable ACES tonemapping for centroid conversion ---
